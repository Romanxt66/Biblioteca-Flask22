...
def add():
    if request.method == 'POST':
        ...
        new_user = User(
            nameUser=nameUser,
            emailUser=emailUser,
            passwordUser=generate_password_hash(passwordUser)
        )
...
def register():
    if request.method == 'POST':
        ...
        errors = User.validate_registration(nameUser, emailUser, passwordUser)
        if errors:
    ...
    return render_template('users/register.html', errors=[], nameUser='', emailUser='')
...
def edit(id):
    ...
    if request.method == 'POST':
        ...
        user.emailUser   = request.form['emailUser']
        user.passwordUser = generate_password_hash(request.form['passwordUser'])
        db.session.commit()
        return redirect(url_for('user.index'))
